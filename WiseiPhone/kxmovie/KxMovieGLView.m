...
//- (void)getwh:(NSNotification*)info
//{
//    _backingWidth=[[[info userInfo] valueForKey:@"wth"] intValue];
//    _backingHeight=[[[info userInfo]valueForKey:@"hgt"] intValue];
//
//}

- (void)layoutSubviews
{
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getwh:) name:@"changeframe" object:nil];
    ...
    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
	if (status != GL_FRAMEBUFFER_COMPLETE) {
	//	 [[NSNotificationCenter defaultCenter] postNotificationName:@"failplay" object:self];
        NSLog(@"failed to make complete framebuffer object %x", status);
        
	} else {
//        NSNumber * wid=[NSNumber numberWithInt:_backingWidth];
//        NSDictionary * dic=[NSDictionary dictionaryWithObjectsAndKeys:wid,@"width", nil];
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"frame" object:self userInfo:dic];
//        NSLog(@"OK setup GL framebuffer %d:%d", _backingWidth, _backingHeight);
        [[NSNotificationCenter defaultCenter] postNotificationName:@"beginplay" object:self];
        isinterest=0;
    }
