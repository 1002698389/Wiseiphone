...
static NSString * errorMessage (kxMovieError errorCode)
{
    ...
    switch (errorCode) {
        ...
        case kxMovieErrorOpenFile:
            return NSLocalizedString(@"不能打开视频,请检查网络", nil);
            
        case kxMovieErrorStreamInfoNotFound:
            return NSLocalizedString(@"没有找到视频", nil);
            
        case kxMovieErrorStreamNotFound:
            return NSLocalizedString(@"没有收到视频信息", nil);
            
        case kxMovieErrorCodecNotFound:
            return NSLocalizedString(@"没有找到编码信息", nil);
            
        case kxMovieErrorOpenCodec:
            return NSLocalizedString(@"不能打开编码", nil);
                             ...
                             error: (NSError **) perror
{
    ...
    if (mp) {
      ...
      NSLog(@"打开是否出错=%d",[mp openFile:path error:perror]);
    return mp;
}
