...
static NSString * formatTimeInterval(CGFloat seconds, BOOL isLeft)
{
    ...
    return [NSString stringWithFormat:@"%@%d:%0.2d:%0.2d", isLeft ? @"-" : @"", (int)h,(int)m,(int)s];
}
...
- (void)loadView
{
   ...
   _messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(20,40,width-40,40)];
    ...
    _bottomHUD.frame = CGRectMake(30,height-(55),width-(30*2),55);
...
- (void)clearBuffer
{
    if (self.playing) {
        ...
        [_decoder closeFile];
        [_decoder openFile:nil error:nil];
        BOOL iserror=[_decoder openFile:nil error:nil];
...
{
    CGRect bounds = CGRectMake(0, 0, 156, 128.61);
  // CGRect bounds = CGRectMake(0, 0, 228, 188);
    if (_decoder.validVideo) {
        self.glView = [[KxMovieGLView alloc] initWithFrame:bounds decoder:_decoder];
    } 
    ...
    if (_decoder.validVideo) {
        ...
        self.imageView.image = [UIImage imageNamed:@"kxmovie.bundle/music_icon.png"];
...
- (UIView *) frameView
{
    return self.glView ? self.glView : self.imageView;
}
...
- (CGFloat) presentVideoFrame: (KxVideoFrame *) frame
{
    if (self.glView) {
        ...
        self.imageView.image = [rgbFrame asImage];
    }
...
- (void) updateHUD
{
    ...
    NSString *subinfo = _decoder.validSubtitles ? [NSString stringWithFormat: @" %lud",(unsigned long)_subtitles.count] : @"";
    ...
    _messageLabel.text = [NSString stringWithFormat:@"%lud %lud%@ %c - %@ %@ %@\n%@",
