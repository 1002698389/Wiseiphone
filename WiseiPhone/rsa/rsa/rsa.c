...
//  printf("rsa pub encrypt  %d\n", modulusLen-inputLen-1);
  for (i = 2; i < modulusLen - inputLen - 1; i++) {
    /* Find nonzero random byte.
     */
	 //   printf("rsa pub encrypt for %d\n", i);

    do {
			  //  printf("before R_GenerateBytes\n" );

      R_GenerateBytes (&byte, 1, randomStruct);
		// printf("after R_GenerateBytes %d\n", byte );
